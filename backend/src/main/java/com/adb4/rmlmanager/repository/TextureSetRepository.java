 ...
 * with dependent texture sets instead of failing on the foreign key; extended
 * by KAN-22 with the variant lookup used for version bookkeeping.
 */
public interface TextureSetRepository extends JpaRepository<TextureSet, UUID> {

    List<TextureSet> findByVariantId(UUID variantId);
